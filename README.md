...
選択した色は 20 行目あたりの `color` という変数(データ)の中に格納されているので、この値をキューブに反映させてみよう！  
キューブの色を決めるコードは 112 行目あたり、最初は白色(カラーコード `#FFFFFF` )の固定値で書かれているよ(この行の `color=` は変数 `color` とは別物なので注意)。  
